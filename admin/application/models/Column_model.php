...
class Column_model extends CI_Model{
	 ...
	 * @return [type] [description]
	 */
	public function get_post_list ($id=0, $page=1, $page_size=5, $page_count=1, $count=0, $top_id=array(),$top_list=array() )
	{	
	    	    
	    if ($page <= 1) {
	    	//剩余首页非置顶帖信息
	    	$offset = $page_size*($page-1);
	    	$list = $this->db->select('id,user_id,cate_id,title,reply_num,topdated,is_ban,update_dated')->where('cate_id',$id)->where_not_in('id', $top_id)->order_by('update_dated','desc')->get('posts', $page_size-$count, $offset)->result_array();
			//合并数据
			$data = $this->handleList($list, $top_list);
	    }  else if ($page > $page_count) {
	    		$offset = $page_size*($page_count-1);
		    	$list = $this->db->select('id,user_id,cate_id,title,reply_num,topdated,is_ban,update_dated')->where('cate_id',$id)->where_not_in('id', $top_id)->order_by('update_dated','desc')->get('posts', $page_size, $offset)->result_array();	
